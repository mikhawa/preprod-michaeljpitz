...
Portfolio CV + blog personnel en Symfony 7.4 LTS.

**Stack** : PHP 8.3 / Symfony 7.4 / Doctrine ORM 3.x / Twig 3.x / MariaDB 10.11 / AssetMapper / Tailwind CSS 3.x / Stimulus.js + Turbo / EasyAdmin 4.x / VichUploaderBundle / Mailpit (dev) / Mailjet (prod)
...
Toujours demander confirmation à l'utilisateur avant de modifier ou créer un fichier,
quelle que soit la tâche. Proposer le plan envisagé, attendre la validation explicite,
puis seulement agir.

### Langue
Tout en français : commentaires de code, messages de commit, documentation, réponses Claude.
...
**Claude peut et doit choisir lui-même le modèle le plus adapté** à chaque tâche, sans attendre que l'utilisateur le précise. Si la tâche en cours relève d'un modèle différent du modèle actif, Claude le signale et utilise l'`Agent` tool avec le paramètre `model` approprié (`opus`, `sonnet` ou `haiku`).
